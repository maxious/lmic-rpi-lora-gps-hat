...
static const u1_t DEVKEY[16] = {0x63, 0x68, 0x3E, 0xF7, 0x53, 0xBF, 0x15, 0x3B, 0x94, 0xD4, 0xF9, 0x37, 0x9A, 0x56,
                                0x7E, 0xAD};

// LoRaWAN AppSKey, application session key
// Use this key to get your data decrypted by The Things Network
static const u1_t ARTKEY[16] = {0x20, 0xE6, 0xE3, 0xB0, 0x7D, 0x35, 0xC8, 0x7E, 0xFE, 0x7F, 0x8D, 0xE1, 0x23, 0xA6,
                                0x9A, 0x5E};
...
static void do_gps_read(osjob_t *j) {
        ...
        if ((rc = gps_read(&gps_d)) == -1) {
            ...
            if ((gps_d.status == STATUS_FIX) &&
                ...
                printf("latitude: %f, longitude: %f, altitude: %f, hdop: %f, timestamp: %lf\n", gps_d.fix.latitude,
                       gps_d.fix.longitude, gps_d.fix.altitude, gps_d.dop.hdop, gps_d.fix.time);
                ...
                printf("\nPKT latitude: %f, longitude: %f, altitude: %f, hdop: %f\n",
                       floor(gps_d.fix.latitude * 1000000), floor(gps_d.fix.longitude * 1000000),
                       floor(gps_d.fix.altitude), floor(gps_d.dop.hdop * 100));
            } else {
    ...
    if (coords[0] == '!') {
        /* wait for .5 seconds to receive data */
        if (gps_waiting(&gps_d_alt, 500000)) {
            /* read data */
            if ((rc = gps_read(&gps_d_alt)) == -1) {
                printf("error occured reading gps data. code: %d, reason: %s\n", rc, gps_errstr(rc));
            } else {
                /* Display data from the GPS receiver. */
                if ((gps_d_alt.status == STATUS_FIX) &&
                    (gps_d_alt.fix.mode == MODE_2D || gps_d_alt.fix.mode == MODE_3D) &&
                    !isnan(gps_d_alt.fix.latitude) &&
                    !isnan(gps_d_alt.fix.longitude)) {
                    printf("latitude: %f, longitude: %f, altitude: %f, hdop: %f, timestamp: %lf\n", gps_d_alt.fix.latitude,
                           gps_d_alt.fix.longitude, gps_d_alt.fix.altitude, gps_d_alt.dop.hdop, gps_d_alt.fix.time);
                    int32_t lat = gps_d_alt.fix.latitude * -10000;
                    int32_t lon = gps_d_alt.fix.longitude * 10000;
                    ...
                    coords[6] = floor(gps_d_alt.fix.altitude);
                    coords[7] = gps_d_alt.dop.hdop * 100;
                    for (int i = 0; i < 8; i++) {
                        printf("%02X ", coords[i]);
                    }
                    printf("\nPKT latitude: %f, longitude: %f, altitude: %f, hdop: %f\n",
                           floor(gps_d_alt.fix.latitude * 1000000), floor(gps_d_alt.fix.longitude * 1000000),
                           floor(gps_d_alt.fix.altitude), floor(gps_d_alt.dop.hdop * 100));
                } else {
                    printf("no GPS data available, hdop: %f\n", gps_d_alt.dop.hdop);
                    coords[0] = '!';
                }
...
void setup() {
    ...
    if ((rc = gps_open("localhost", "2948", &gps_d)) == -1) {
        printf("GPS ERROR code: %d, reason: %s\n", rc, gps_errstr(rc));
    }
    gps_stream(&gps_d_alt, WATCH_ENABLE | WATCH_JSON, NULL);
